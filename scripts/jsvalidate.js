...
/*
		...
		* required (not blank)
		* validate-length (user enter value should not exceed length.)
...
function FIC_checkForm(e) {
	...
	if (!all_valid) {
		if (errs.length > 0){
			...
			alert('Some required values are not correct.');
		}
...
function attachToForms(e) {
	...
	for(var i=0;i<frms.length;i++) {
		...
		YAHOO.util.Event.addListener(frms[i], "submit", replaceControlChars);
		// added by Insta: not in the original
		captureFormSubmit(frms[i]);
	}
}

// added by Insta: not in the original
function captureFormSubmit(form) {
	// debug("Capturing form submit: " + form.name);
	form.oldSubmit = form.submit;

	form.submit = function validatedSubmit() {
		replaceControlChars(form);
		var validate = form.validate ? form.validate.value : 'Y';
		if(validate == 'Y') {
			var valid = FIC_checkForm(form);
			if (valid) form.oldSubmit();
		}else {
			form.oldSubmit();
		}
	};
...
/*
* this function replaces the non printable characters with the empty string
* from the text and textarea elements.
*/
function replaceControlChars(e) {
	...
	var textarea = elm.getElementsByTagName("textarea");
	for (var i=0; i<inputs.length; i++) {
		if (inputs[i].type == 'text') {
			//matches all control characters including null excluding the new line (i.e., line feed) character
			inputs[i].value = inputs[i].value.replace(/[\x00-\x09\x0b-\x1f]/g, ' ');
		}
	}

	for (var j=0; j<textarea.length; j++) {
		//matches all control characters including null excluding the new line (i.e., line feed) character
		textarea[j].value = textarea[j].value.replace(/[\x00-\x09\x0b-\x1f]/g, ' ');
	}
